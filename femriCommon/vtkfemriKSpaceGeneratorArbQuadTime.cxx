/*=========================================================================
  ...
  Program:   FEMRI
  Module:    $RCSfile: vtkfemriKSpaceGeneratorArbQuadTime.cxx,v $
  Language:  C++
...
vtkCxxRevisionMacro(vtkfemriKSpaceGeneratorArbQuadTime, "$Revision: 1.2 $");

vtkfemriKSpaceGeneratorArbQuadTime::vtkfemriKSpaceGeneratorArbQuadTime()
{
...
int vtkfemriKSpaceGeneratorArbQuadTime::RequestInformation (
...
double vtkfemriKSpaceGeneratorArbQuadTime::ComputeVoxelVolume()
{
...
int vtkfemriKSpaceGeneratorArbQuadTime::RequestData(
    vtkInformation* vtkNotUsed( request ),
    ...
    vtkInformationVector* outputVector)
{
  ...
  if (!writer)
  { 
	cout << "femri Error: error opening file for output" << endl;
	return 0.0;
  }
  ...
  for (ijk[2]=extent[4]; ijk[2]<=extent[5]; ijk[2]++)
    ...
    for (ijk[1]=extent[2]; ijk[1]<=extent[3]; ijk[1]++)
      ...
      for (ijk[0]=extent[0]; ijk[0]<=extent[1]; ijk[0]++)
        ...
        this->EvaluateFourierFunction(frequency,fourierValue, writer);
...
void vtkfemriKSpaceGeneratorArbQuadTime::PrintSelf(ostream& os, vtkIndent indent)
{
